...
public class CategorySpecification {
	...
	public static Specification<Goods> filterBy(String main, List<String> mid, List<String> detail, List<String> gender,
			List<String> color, List<String> print, Integer minPrice, Integer maxPrice, String keyword) {
		Specification<Goods> spec = (root, query, cb) -> cb.conjunction(); // 빈 조건에서 시작

		if (main != null && !main.isEmpty())
			spec = spec.and(hasMain(main));
		if (mid != null && !mid.isEmpty())
			spec = spec.and(hasMidIn(mid));
		if (detail != null && !detail.isEmpty())
			spec = spec.and(hasDetailIn(detail));
		if (gender != null && !gender.isEmpty())
			spec = spec.and(hasGender(gender));
		if (color != null && !color.isEmpty())
			spec = spec.and(hasColorIn(color));
		if (print != null && !print.isEmpty())
			spec = spec.and(hasPrintIn(print));
		if (minPrice != null || maxPrice != null)
			spec = spec.and(hasPriceBetween(minPrice, maxPrice));
		if (keyword != null && !keyword.isBlank())
			spec = spec.and(hasProductName(keyword)); // 키워드 검색 조건 추가
